...
```python
entidades = anonymizer.extract_entities(texto_original)
print(entidades)
# Saída esperada:
...
```python
texto_anonimizado = anonymizer.anonymize_text(texto_original, entidades)
...
entidades = anonymizer.extract_entities(text_or_path=texto_original, return_type="label_position")
print(entidades)
...
anonymizer = Anonimizar(model_path=model_path, labels=['CPF', 'PLACA'])
...
```python
import pandas as pd
from anonimizar import Anonimizar, Trainer
...
mix_data = {
    'text': "Erivan, cujo documento é 123.456.789-09, tem um fusca azul com a placa ERI-1007.",
    'entities': [(25,39,'CPF'), (80,85,'PLACA')]  # PLACA fora do tamanho do texto
}
```
...
try:
    ...
    print(e)
# Esperado: "Posições inválidas: 80-85 em texto de tamanho 80"
```
...
```python
trainer.training_data = []
trainer.add_data(mix_data, errors='ignore', strict_clean=False)
trainer.training_data
# Esperado: mantém as duas anotações, inclusive o offset inválido.
# Use errors='coerce' para remover a entidade inválida.
```
...
```python
trainer.training_data = []
trainer.add_data(mix_data, errors='coerce', strict_clean=False)
trainer.training_data
# Esperado: mantém CPF; PLACA inválida é removida com warning
```
...
trainer.add_data(data, errors="ignore", auto_clean=True, normalize_entities=False)
# Entidades são mantidas com espaços

# errors='coerce' + auto_clean=True - corrige espaços
data = [{"text": "CPF: 123.456.789-00  ", "entities": [(5, 21, "CPF")]}]
trainer.add_data(data, errors="coerce", auto_clean=True, normalize_entities=False)
# Entidades são corrigidas: (5, 21) -> (5, 19)

# errors='raise' + auto_clean=True - levanta erro se não conseguir corrigir
data = [{"text": "EMAIL: teste@exemplo.com", "entities": [(7, 24, "EMAIL")]}]
trainer.add_data(data, errors="raise", auto_clean=True, normalize_entities=False)
...
_por_documento = [
    ...
    },
    {
        ...
        'entities': [(23,37,'CPF'), (69,77,'PLACA')]
    },
    {
...
trainer.add_data(df_documento, errors='coerce', keep_empty_entities=False, strict_clean=False)
trainer.training_data
# Esperado: os quatro textos com CPF/PLACA são mantidos; o texto sem entidade
# e o texto com EMAIL fora da configuração são descartados.
```
...
_por_entidade = {
    ...
    'start': [25, 71, 36, 23, 69, 36, 28],
    'end': [39, 79, 44, 37, 77, 44, 50],
...
trainer.training_data = []
trainer.add_data(df_entidade, errors='coerce', keep_empty_entities=False, strict_clean=False)
trainer.training_data
