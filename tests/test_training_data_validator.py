...
class TestSplitEntitiesAtNewline:
    """Testes para _split_entities_at_newline."""

    def test_ignores_entity_without_newline(self) -> None:
        """Entidade sem quebra de linha permanece inalterada."""
        entities = [(0, 5, "ENDEREÇO")]
        result = _split_entities_at_newline("Rua A", entities)
        assert result == entities

    def test_splits_on_newline(self) -> None:
        """Entidade com \\n vira 2 entidades com offsets corretos."""
        text = "Rua A\nCEP: 123"
        entities = [(0, 16, "ENDEREÇO")]
        result = _split_entities_at_newline(text, entities)
        assert len(result) == 2
        assert result[0] == (0, 5, "ENDEREÇO")
        assert result[1] == (6, 14, "ENDEREÇO")

    def test_ignores_consecutive_newlines(self) -> None:
        """\\n\\n consecutivos não geram entidade vazia."""
        text = "Rua A\n\nCEP: 123"
        entities = [(0, 17, "ENDEREÇO")]
        result = _split_entities_at_newline(text, entities)
        assert len(result) == 2
        assert result[0] == (0, 5, "ENDEREÇO")
        assert result[1] == (7, 15, "ENDEREÇO")

    def test_handles_crlf(self) -> None:
        """\\r\\n é tratado como separador único de 2 chars."""
        text = "Rua A\r\nCEP: 123"
        entities = [(0, 17, "ENDEREÇO")]
        result = _split_entities_at_newline(text, entities)
        assert len(result) == 2
    ...
    def test_handles_cr(self) -> None:
        """\\r isolado é tratado como separador de 1 char."""
        text = "Rua A\rCEP: 123"
        entities = [(0, 15, "ENDEREÇO")]
        result = _split_entities_at_newline(text, entities)
        assert len(result) == 2
        assert result[0] == (0, 5, "ENDEREÇO")
        assert result[1] == (6, 14, "ENDEREÇO")
    ...
    def test_handles_mixed_variants(self) -> None:
        """\\n\\r gera separadores distintos."""
        text = "a\n\rb"
        entities = [(0, 4, "ENDEREÇO")]
        result = _split_entities_at_newline(text, entities)
        assert len(result) == 2
        assert result[0] == (0, 1, "ENDEREÇO")
        assert result[1] == (3, 4, "ENDEREÇO")
