"""Testes para o módulo SeiAnonimizarNERTrainer."""
...
class TestTrainerInit:
    ...
    def test_init_with_model(self, model_path, temp_dir):
        ...
        trainer = SeiAnonimizarNERTrainer(model_name=model_path, output_dir=str(temp_dir / "custom_model"))
    ...
    def test_init_custom_labels(self):
        ...
        trainer = SeiAnonimizarNERTrainer(labels=custom_labels)
    ...
    def test_init_raises_when_model_not_found(self, tmp_path):
        with pytest.raises(Exception):
            SeiAnonimizarNERTrainer(model_name=str(tmp_path / "modelo_inexistente"))


class TestDefaultLogger:
    def test_logger_singleton_handlers(self, caplog):
        trainer = SeiAnonimizarNERTrainer()
...
class TestValidateDataPolicies:
    def test_validate_data_label_invalido_raise(self):
        ...
        with pytest.raises(ValueError, match="Label inválido"):
            trainer._validate_data(data, errors="raise")

    def test_validate_data_posicao_invalida_coerce(self):
        trainer = SeiAnonimizarNERTrainer()
        text = "CPF 123.456.789-09"
        data = [(text, {"entities": [(0, 100, "CPF")]})]
        out = trainer._validate_data(data, errors="coerce", keep_empty_entities=True)
        assert len(out) == 1
    ...
    def test_validate_data_posicao_invalida_ignore(self):
        ...
        text = "CPF 123.456.789-09"
        data = [(text, {"entities": [(0, 100, "CPF")]})]
        out = trainer._validate_data(data, errors="ignore", keep_empty_entities=True)
        assert len(out) == 1
...
class TestValidateBiluoPolicies:
    def test_biluo_raise(self):
        ...
        with pytest.raises(ValueError, match="Erro BILUO"):
            trainer._validate_biluo_tags(text, [(start, start + 3, "EMAIL")], errors="raise")

    def test_biluo_coerce(self):
        trainer = SeiAnonimizarNERTrainer()
        text = "EMAIL: teste@exemplo.com"
        start = text.index("exemplo")
        out = trainer._validate_biluo_tags(text, [(start, start + 3, "EMAIL")], errors="coerce")
        assert out == []
    ...
    def test_biluo_ignore(self):
        ...
        text = "EMAIL: teste@exemplo.com"
        start = text.index("exemplo")
        out = trainer._validate_biluo_tags(text, [(start, start + 3, "EMAIL")], errors="ignore")
...
class TestTransformFromPandas:
    def test_transform_row_per_entity_format(self):
        ...
        out = trainer._transform_data_from_pandas(df)
        assert len(out) == 2
    ...
    def test_transform_invalid_format_raises(self):
        ...
        with pytest.raises(ValueError, match="DataFrame deve ter colunas"):
            trainer._transform_data_from_pandas(df)
...
class TestTraining:
    ...
    def test_train_basic(self, temp_dir, sample_training_data):
        ...
        trainer = SeiAnonimizarNERTrainer(output_dir=str(temp_dir))
        trainer.add_data(sample_training_data)
...
class TestJSONLSupport:
    ...
    def test_load_from_doccano_jsonl(self, temp_dir):
        ...
        data = [
            {"text": "CPF 123.456.789-09 email@test.com", "labels": [[4, 18, "CPF"], [19, 33, "EMAIL"]]},
        ...
        assert len(loaded) == 2
        assert loaded[0][0] == "CPF 123.456.789-09 email@test.com"
    ...
    def test_save_to_doccano_jsonl(self, temp_dir):
        ...
        trainer.add_data(
            [
                {"text": "CPF 123.456.789-09 email@test.com", "entities": [(4, 18, "CPF"), (19, 33, "EMAIL")]},
                {"text": "CPF 987.654.321-00", "entities": [(4, 18, "CPF")]},
            ]
        )
        ...
        assert item1["text"] == "CPF 123.456.789-09 email@test.com"
    ...
    def test_load_jsonl_to_dataframes(self, temp_dir):
        ...
        df_textos, df_entidades = trainer._load_jsonl_to_dataframes(str(jsonl_path))
