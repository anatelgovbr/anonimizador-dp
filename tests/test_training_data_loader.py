...
class TestTransformStandardFormat:
    ...
    def test_entities_validas(self, logger) -> None:
        ...
        df = pd.DataFrame({"text": ["texto"], "entities": [[(0, 5, "CPF")]]})
        result = transform_standard_format(df, logger)
        assert len(result) == 1
    ...
    def test_entities_nao_lista_ignorada(self, logger) -> None:
        ...
        df = pd.DataFrame({"text": ["texto_invalido", "texto_valido"], "entities": ["nao_lista", [(0, 3, "RG")]]})
        result = transform_standard_format(df, logger)
...
class TestTransformRowPerEntityFormat:
    ...
    def test_formato_basico(self, logger) -> None:
        ...
        result = transform_row_per_entity_format(df, logger)
        assert len(result) == 1
...
class TestConvertInputData:
    ...
    def test_dataframe_sem_fn_lanca_erro(self, logger) -> None:
        ...
        df = pd.DataFrame({"text": ["texto"], "entities": [[(0, 3, "CPF")]]})
        with pytest.raises(ValueError, match=r"transform_pandas_fn"):
            convert_input_data(df, logger)
    ...
    def test_dataframe_com_fn(self, logger) -> None:
        ...
        result = convert_input_data(df, logger, transform_pandas_fn=fn)
        assert len(result) == 1
