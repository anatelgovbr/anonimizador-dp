...
@pytest.fixture
def anonymizer(model_path) -> Anonimizar:
    """Fixture para Anonimizar configurado."""
...
@pytest.fixture
def anonymizer_rne(model_path) -> Anonimizar:
    """Fixture com foreign_rg=True para detectar RNE/CRNM."""
    anon = Anonimizar(model_path=model_path)
    anon.add_apply_patterns(["RG"], foreign_rg=True)
    return anon


@pytest.fixture
def geo_anonymizer(model_path) -> Anonimizar:
    """Anonimizar configurado apenas com padrões GEO_COORD."""
    anon = Anonimizar(model_path=model_path, auto_patterns=False)
    anon.add_apply_patterns(["GEO_COORD"])
    return anon
