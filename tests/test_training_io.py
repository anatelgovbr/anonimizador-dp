...
class TestLoadJSONLToDataFrames:
    ...
    def test_arquivo_sem_registros_validos_retorna_dataframes_sem_colunas(self, tmp_path) -> None:
        """Arquivo sem registros válidos preserva o contrato de DataFrames vazios."""
        f = tmp_path / "vazio.jsonl"
        f.write_text('{"text": "", "labels": []}', encoding="utf-8")

        df_textos, df_entidades = load_jsonl_to_dataframes(f)

        assert df_textos.empty
        assert df_entidades.empty
        assert df_textos.columns.tolist() == []
        assert df_entidades.columns.tolist() == []
