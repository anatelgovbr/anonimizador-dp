...
class TestCalculateOverlap:
    """Testes para função calculate_overlap."""
    ...
    def test_partial_overlap_half(self) -> None:
        """Overlap parcial de 50%."""
        assert calculate_overlap(0, 10, 5, 15) == pytest.approx(_OVERLAP_HALF)

    def test_partial_overlap_one_third(self) -> None:
        """Overlap parcial de 1/3."""
        assert calculate_overlap(0, 9, 6, 15) == pytest.approx(_OVERLAP_ONE_THIRD)

    def test_pred_fully_inside_true(self) -> None:
        """Span predito contido no verdadeiro: overlap baseado no span verdadeiro."""
        # interseção = [5,8], true_length = 10, overlap = 3/10
        assert calculate_overlap(0, 10, 5, 8) == pytest.approx(0.3)

    def test_true_fully_inside_pred(self) -> None:
        """Span verdadeiro contido no predito: overlap 1.0."""
        assert calculate_overlap(5, 8, 0, 10) == _OVERLAP_FULL

    def test_touching_spans(self) -> None:
        """Spans que se tocam exatamente mas não sobrepõem: overlap 0.0."""
        assert calculate_overlap(0, 10, 10, 20) == _OVERLAP_ZERO

    def test_invalid_true_start_gt_end(self) -> None:
        """start_true > end_true levanta ValueError."""
        with pytest.raises(ValueError, match="não pode iniciar depois"):
            calculate_overlap(10, 5, 0, 10)

    def test_invalid_pred_start_gt_end(self) -> None:
        """start_pred > end_pred levanta ValueError."""
        with pytest.raises(ValueError, match="não pode iniciar depois"):
            calculate_overlap(0, 10, 15, 5)

    def test_zero_length_true_span(self) -> None:
        """Span verdadeiro de comprimento zero retorna 0.0 (divisão por zero protegida)."""
        assert calculate_overlap(5, 5, 0, 10) == _OVERLAP_ZERO

    def test_large_positions(self) -> None:
        """Posições grandes não causam problemas numéricos."""
        assert calculate_overlap(1000, 2000, 1000, 2000) == _OVERLAP_FULL
