"""Testes para o módulo Evaluation."""
...
class TestEvaluationInit:
    ...
    def test_init_custom_params(self):
        ...
        evaluator = Evaluation(overlap_threshold=0.7, beta=1.0)
    ...
    def test_init_invalid_threshold(self):
        ...
        with pytest.raises(ValueError, match="overlap_threshold deve estar entre 0.0 e 1.0"):
            Evaluation(overlap_threshold=1.5)
    ...
    def test_prediction_methods_document_anonymizer_type(self):
        """Garante que as anotações públicas não declarem anonimizador como None."""
        extract_annotation = inspect.signature(Evaluation.extract_predictions).parameters["anonymizer"].annotation
        evaluate_annotation = inspect.signature(Evaluation.evaluate_model).parameters["anonymizer"].annotation

        assert extract_annotation == "Anonimizar"
        assert evaluate_annotation == "Anonimizar | None"
...
class TestPredictionsIO:
    ...
    def test_save_and_load_predictions_roundtrip(self, tmp_path, fmt, ext):
        ...
        assert {"id", "tp_entidade", "start_entidade", "end_entidade"}.issubset(loaded.columns)
...
class TestLoadDataFromFiles:
    def test_load_data_from_parquet_and_remove(self, tmp_path):
        ...
        evaluator = Evaluation(texts_path=str(texts_p), ground_truth_path=str(gt_p))
        assert evaluator.df_texts is not None
        assert evaluator.df_ground_truth is not None
    ...
    def test_load_data_from_files_missing_cols(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Colunas ausentes"):
            Evaluation(texts_path=str(texts_p), ground_truth_path=str(gt_p))
    ...
    def test_load_data_from_files_unsupported_format(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Formato não suportado"):
            Evaluation(texts_path=str(t), ground_truth_path=str(g))
...
class TestOverlapCalculation:
    """Testes para cálculo de sobreposição (IoU)."""

    @pytest.mark.parametrize(
        ("start_true", "end_true", "start_pred", "end_pred", "expected"),
        [
            (0, 10, 0, 10, 1.0),  # Sobreposição completa: IoU = 10/10
            (0, 10, 5, 15, 1 / 3),  # Sobreposição parcial: IoU = 5/15
            (0, 10, 10, 20, 0.0),  # Sem sobreposição
            (0, 10, 2, 8, 0.6),  # Predição dentro da verdade: IoU = 6/10
            (2, 8, 0, 10, 0.6),  # Verdade dentro da predição: IoU = 6/10
        ],
    )
...
class TestModelEvaluation:
    ...
    def test_evaluate_model_basic(self, sample_evaluation_data, anonymizer):
        ...
        for _entity_type, metrics in results.items():
...
class TestEvaluateMultipleThresholds:
    def test_multiple_thresholds_with_predictions(self, sample_evaluation_data):
        ...
        result_df = evaluator.evaluate_multiple_thresholds(
            predictions=preds, overlap_thresholds=[0.7, 0.9], beta_values=[1, 2]
        )
        assert isinstance(result_df, pd.DataFrame)
        assert not result_df.empty
        assert {"tp_entidade", "fbeta", "precision", "recall", "overlap_threshold", "beta"}.issubset(result_df.columns)
