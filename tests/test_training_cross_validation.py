...
class TestMakeFoldsByID:
    ...
    def test_usa_coluna_id_doc(self, logger) -> None:
        ...
        folds = make_folds_by_id(df, n_splits=3, random_state=None, shuffle=False, logger=logger)
        assert len(folds) == 3
...
class TestMakeStratifiedFoldsByID:
    ...
    def test_sobra_distribuida(self, logger) -> None:
        ...
        folds = make_stratified_folds_by_id(
            df, features=["CPF"], n_splits=3, random_state=0, shuffle=False, logger=logger
        )
        assert len(folds) == 3
    ...
    def test_split_size_zero_distribui_individualmente(self, logger) -> None:
        ...
        folds = make_stratified_folds_by_id(
            df, features=["CPF"], n_splits=5, random_state=0, shuffle=False, logger=logger
        )
        assert len(folds) == 5
