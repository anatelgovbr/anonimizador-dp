...
from anonimizar._constants.entity_labels import ALL_MODEL_LABELS
from anonimizar._patterns.builtin import (
    PATTERN_ADDERS,
    add_pattern_passaporte_est,
    add_pattern_rg_estrangeiro,
)
...
def add_apply_patterns(  # noqa: C901, PLR0912
    ...
    *,
    ...
    Args:
        ...
        foreign_rg (bool): Se True, adiciona padrões de RG estrangeiro (RNE/CRNM)
    ...
    """
    logger.debug(
        "Aplicando padrões: labels=%s, replace_patterns=%s, use_model_labels=%s, foreign_passport=%s, foreign_rg=%s",
        labels,
        replace_patterns,
        use_model_labels,
        foreign_passport,
    ...
    for label in labels:
        if label in PATTERN_ADDERS:
            ...
            if label == "RG" and foreign_rg:
                logger.debug("Adicionando padrões de RG estrangeiro (RNE/CRNM)")
                add_pattern_rg_estrangeiro(patterns, logger)
            before = len(patterns)
        ...
        elif label in ALL_MODEL_LABELS:
            logger.debug("Rótulo '%s' reconhecido mas sem padrões regex (ignorado)", label)
        else:
...
def list_patterns(patterns: list[dict], model_labels: list[str]) -> dict:
    ...
    Args:
        ...
        model_labels (list[str]): Labels ativos no modelo. São o critério atual
            de classificação, não a origem de cada padrão.

    Returns:
        dict: Dicionário com duas chaves:
            - "builtin": Padrões cujo label está em ``model_labels``
            - "custom": Padrões cujo label não está em ``model_labels``

    Note:
        O registro não preserva a origem de um padrão. Portanto, um padrão
        built-in pode aparecer em ``custom`` se seu label não estiver ativo no
        modelo, e o inverso também é possível.
    """
