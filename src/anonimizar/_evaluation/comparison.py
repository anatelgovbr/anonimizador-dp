...
def compare_reports(
    ...
    Examples:
        ...
        >>> current = pd.DataFrame({"tp_entidade": ["CPF"], "fbeta": [0.9], "precision": [0.9], "recall": [0.9]})
        >>> previous = pd.DataFrame({"tp_entidade": ["CPF"], "fbeta": [0.8], "precision": [0.8], "recall": [0.8]})
        >>> compare_reports(current, previous, logger)["fbeta_melhorou"].tolist()
        [True]
    """
...
def classify_cases(comparison_data: pd.DataFrame) -> pd.DataFrame:
    ...
    Examples:
        >>> comparison = pd.DataFrame({"y_true": [1, 0], "y_pred": [1, 0]})
        >>> classify_cases(comparison)["classification"].tolist()
        ['TP', 'TN']
    """

    def _classify(row: pd.Series) -> str:
        """Classifica uma linha de comparação pelos indicadores binários."""
...
def get_classification_cases(
    ...
    The returned DataFrame includes ``classification``, ``has_gt_text`` and
    ``has_pred_text`` in addition to the comparison columns.
    """
...
def get_error_analysis(
    ...
    Args:
        comparison_data: DataFrame com as colunas ``y_true`` e ``y_pred``.
            A classificação é calculada internamente.
