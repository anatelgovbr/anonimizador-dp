...
a lógica de cross-validation, separando-a da facade SeiAnonimizarNERTrainer.
"""
...
class NERCrossValidator:
    ...
    SeiAnonimizarNERTrainer, incluindo criação de folds, treinamento
    por fold, avaliação e agregação de resultados.

    Examples:
        >>> cv = NERCrossValidator(logger=logger)
        >>> results = cv.run(
        ...     df_entidades=df_ents,
        ...     df_textos=df_texts,
        ...     trainer_factory=trainer_factory_fn,
        ...     n_splits=5,
        ... )
    """
    ...
    def run(  # noqa: PLR0915, C901
        ...
        *,
        ...
        Args:
            ...
            trainer_factory: Callable que cria instância de SeiAnonimizarNERTrainer,
        ...
        def run_single_fold(
            ...
            try:
                from anonimizar.sei_anonimizar import SeiAnonimizar  # noqa: PLC0415
                from anonimizar.sei_anonimizar_evaluation import SeiAnonimizarEvaluation  # noqa: PLC0415

                model_path = str(fold_dir / "model")
                anonymizer = SeiAnonimizar(model_path, labels=supported_labels)
                ...
                self.logger.info("Fold %d concluído", fold_idx)
                ...
                if df_holdout_texts is not None and df_holdout_gt is not None:
                    ...
                    evaluator_holdout = SeiAnonimizarEvaluation(
                        overlap_threshold=eval_params.get("overlap_threshold", DEFAULT_OVERLAP_THRESHOLD),
                        beta=eval_params.get("beta", DEFAULT_BETA),
                    )
