...
def split_data(
    ...
    logger.info("Dados divididos: %d treino, %d validação", len(train_data), len(val_data))
...
def val_data_to_evaluation(
    ...
    """Converte dados de validação para formato do Evaluation.
...
def convert_input_data(  # noqa: C901, PLR0912
    data: list | dict | pd.DataFrame | str,
    logger: logging.Logger,
    transform_pandas_fn: Callable[[pd.DataFrame], list[tuple[str, dict]]] | None = None,
    load_jsonl_fn: Callable[[str], list[tuple[str, dict]]] | None = None,
    errors: str = "raise",
) -> list[tuple[str, dict]]:
    ...
    if isinstance(data, list):
        for case in data:
            if "labels" in case and "entities" not in case:
                ...
                for item in case["labels"]:
                    if isinstance(item, list | tuple) and len(item) >= _MIN_LABEL_ITEMS:
                ...
                logger.warning("Caso sem 'entities' ou 'labels': %s", list(case.keys()))
        ...
        if "labels" in data and "entities" not in data:
            ...
            logger.warning("Caso sem 'entities' ou 'labels': %s", list(data.keys()))
        ...
        if data_path.suffix.lower() == ".jsonl":
            logger.info("Detectado arquivo JSONL: %s", data)
...
def apply_auto_clean(
    ...
    clean_entities_fn: Callable[..., list[tuple[int, int, str]]],
    *,
    strict_clean: bool,
    ...
    Args:
        ...
        clean_entities_fn: Callback de limpeza chamado como
            ``clean_entities_fn(text, entities, strict=..., resolve_conflicts=...,
            errors=...)``. Deve retornar a lista de entidades limpas. O callback
            pode levantar ``ValueError``; erros que mencionam ``"Conflitos de
            entidades"`` descartam o exemplo e os demais são propagados.
    ...
    """
    mode = "estrita" if strict_clean else "flexível"
    logger.debug("Aplicando limpeza automática %s em %d exemplos...", mode, len(data))
    ...
    for text, annotations in data:
        ...
        try:
            ...
            if strict_clean and len(cleaned_entities) == 0 and original_count > 0:
                ...
                logger.debug(
                    "Exemplo descartado (modo estrito): '%s...' - %d entidades originais não passaram na limpeza",
                    text[:50],
                    original_count,
                )
            elif len(cleaned_entities) == 0 and not keep_empty_entities:
                stats["discarded_empty"] += 1
                logger.debug("Exemplo descartado (vazio): '%s...'", text[:50])
            else:
                cleaned_data.append((text, {"entities": cleaned_entities}))
                stats["kept"] += 1

                if len(cleaned_entities) != original_count:
                    logger.debug("Exemplo mantido com menos entidades: %d -> %d", original_count, len(cleaned_entities))

        except ValueError as e:
            if "Conflitos de entidades" in str(e):
                stats["discarded_conflicts"] += 1
                logger.debug("Exemplo descartado por conflitos: '%s...'", text[:50])
            else:
                raise

    logger.debug(
        "Limpeza concluída: %d mantidos, %d descartados (modo estrito), "
        "%d descartados (vazios), %d descartados (conflitos)",
        stats["kept"],
        stats["discarded_strict"],
        stats["discarded_empty"],
        stats["discarded_conflicts"],
    )
