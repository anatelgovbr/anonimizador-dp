...
def make_folds_by_id(
    ...
    logger.debug(f"Criados {len(folds)} folds com KFold simples")
    return folds
...
def make_stratified_folds_by_id(
    ...
    Returns:
        list: Lista de listas, cada uma contendo IDs para um fold
    """
    id_col = "id_doc" if "id_doc" in df_entidades.columns else "id"
    entity_col = "tp_entidade" if "tp_entidade" in df_entidades.columns else "entidade"

    df_grouped = df_entidades.groupby([id_col, entity_col]).size().unstack(fill_value=0).reset_index()
    ...
    if remaining_ids:
        logger.debug(f"Distribuindo {len(remaining_ids)} documentos restantes entre os folds")
        for doc_id in remaining_ids:
            i = np.argmin(folds_counter)
            folds[i].append(doc_id)
            folds_counter[i] += 1

    logger.debug(f"Criados {len(folds)} folds estratificados com contagens: {folds_counter}")
...
def fold_for_train(folds: list[list], logger: logging.Logger) -> list[tuple[list, list]]:
    ...
    result = list(zip(fold_train, fold_val))
    logger.debug(f"Folds convertidos para treino/validação: {len(result)} pares gerados")
    return result
...
def separate_holdout_test(
    ...
    Args:
        ...
        holdout_test_size: Fração (0-0.5) dos dados para holdout test
        features: Lista de tipos de entidades para estratificação
        random_state: Seed para reprodutibilidade
    ...
    Raises:
        ValueError: Se holdout_test_size não estiver entre 0 e 0.5
    """
    ...
    if holdout_stratify and features and make_stratified_folds_fn:
        logger.info(f"Separando holdout test estratificado ({holdout_test_size:.0%})")
        ...
        logger.info(f"Separando holdout test simples ({holdout_test_size:.0%})")
    ...
    logger.info(f"Holdout test separado: {len(holdout_test_ids)} docs (test), {len(cv_ids)} docs para CV")
    ...
    with open(output_path / "holdout_test_ids.json", "w", encoding="utf-8") as f:
...
def aggregate_cv_results(
    ...
    if summary_metrics:
        ...
        logger.info(
            f"Métricas médias: Precision={avg_metrics['precision']:.3f}, "
            f"Recall={avg_metrics['recall']:.3f}, Fbeta={avg_metrics['fbeta']:.3f}"
        )
    ...
    if holdout_results:
        ...
        with open(output_path / "holdout_test_stats.json", "w") as f:
            json.dump(holdout_stats, f, indent=2)

        logger.info(
            f"Holdout Test - Média: Fbeta={holdout_stats['mean_fbeta']:.3f}±{holdout_stats['std_fbeta']:.3f}, "
            f"Melhor fold: {holdout_stats['best_fold']} (Fbeta={holdout_stats['best_fbeta']:.3f})"
        )
...
def prepare_cv_data(
    ...
    temp_trainer = SeiAnonimizarNERTrainer(model_name=model_name, labels=supported_labels, logger=logger)
    temp_trainer.add_data(df_entidades_treino, **add_data_params)

    cleaned_training_data = temp_trainer.training_data.copy()
    logger.debug(f"Pré-processamento concluído: {len(cleaned_training_data)} exemplos limpos")
...
def prepare_fold_data(
    ...
    logger.debug(f"Fold {fold_idx}: {len(fold_training_data)} exemplos de treino, {len(df_texts_val)} de validação")
...
def save_fold_ids(
    ...
    with open(fold_dir / "fold_ids.json", "w", encoding="utf-8") as f:
    ...
    logger.debug(f"Fold {fold_idx}: IDs salvos - {len(ids_train)} treino, {len(ids_val)} validação")
