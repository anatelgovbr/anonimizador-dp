r"""Módulo SeiAnonimizarEvaluation.

Este módulo oferece **ferramentas completas de avaliação** para modelos NER
especializados em anonimização de dados sensíveis gerados pelo *SeiAnonimizar*.

Principais capacidades

1. **Extração de predições** diretamente de um objeto `SeiAnonimizar`.
...
Classes
- `SeiAnonimizarEvaluation`: classe principal que centraliza todo o workflow de
  avaliação.

Exemplos:

    >>> from anonimizar import SeiAnonimizar, SeiAnonimizarEvaluation
    >>>
...
class SeiAnonimizarEvaluation:
    ...
    def __init__(
        ...
        *,
        ...
        self.logger.info("Inicializando SeiAnonimizarEvaluation")
    ...
    def _normalize_entities(self) -> None:
        """Normaliza entidades usando o mapeamento configurado."""
    ...
    def extract_predictions(
        self, anonymizer: None, save_path: str | None = None, *, force_recompute: bool = False
    ) -> pd.DataFrame:
        """Extrai predições do modelo e opcionalmente salva em arquivo.

        Args:
            anonymizer: Modelo SeiAnonimizar para extração
        ...
        Returns:
            pd.DataFrame: DataFrame com predições
        """
    ...
    def save_predictions(self, output_path: str | Path, format: str = "parquet") -> None:  # noqa: A002
        ...
        Args:
            output_path (str | Path): Caminho para salvar
    ...
    def set_predictions(self, predictions: pd.DataFrame) -> None:
        ...
        # Apply entity mapping after validation
        self.df_predictions["tp_entidade"] = self.df_predictions["tp_entidade"].apply(
            lambda x: self.entity_mapping.get(x, x)
        )
    ...
    def calculate_overlap(self, start_true: int, end_true: int, start_pred: int, end_pred: int) -> float:
        ...
        Delega para `_evaluation.metrics.calculate_overlap()` após validações de NaN.
        ...
        try:
            return metrics.calculate_overlap(start_true, end_true, start_pred, end_pred)
        except ValueError:
            self.logger.exception("A entidade não pode iniciar depois do término da mesma.")
            raise
    ...
    def evaluate_multiple_thresholds(
        ...
        return metrics.evaluate_multiple_thresholds(
